...
public class CalendarModel implements CalendarModelInterface{
	...
	public CalendarModel(DatabaseInterface database)
	{
		this.database = database;
		appointmentList = this.database.initializeDatabase();
	...
	{
		appointmentList.add(new Appointment(date,time, detail, title,type));
		if (this.database!=null)
		{
			this.database.addDatabase(date, time, title, type,detail);
		}
	}
	public void editAppointment(Appointment appointment ,String time,String detail, String title,String type,String titleOld)
	{
		...
		this.database.editDatabase(time, title, detail,type,titleOld);
	}
	public void deleteAppointment(int index)
	{
		Appointment a = this.appointmentList.remove(index);
		this.getNewArray().remove(a);
		this.database.deleteDatabase(a.getTitle());
	}
	...
	public boolean canMarkDay(String date)
	{
		for (int i=0;i<this.getAppointmentList().size();i++)
		{
			...
			if (this.getAppointmentList().get(i).getType().equals("Monthly") && this.getAppointmentList().get(i).getDate().split(" ")[0].equals(date.split(" ")[0]))
			{
				return true;
			}
		}
